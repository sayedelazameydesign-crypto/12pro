/**
 * Edge test for 50MB limit - Gap #4
 * Tests 49MB, 50MB, 51MB at the boundary
 */
...
function createConv() {
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({
      ...
      res.on('end', () => { try { resolve(JSON.parse(data).id); } catch { resolve(JSON.parse(data).conversation?.id || 'conv_test'); } });
    });
    req.on('error', reject);
...
async function run() {
  ...
  for (const t of tests) {
    ...
    const result = {
      test: t.name,
      sizeBytes: t.size,
      sizeMB: (t.size / 1024 / 1024).toFixed(2) + 'MB',
      expectedStatus: t.expected,
      actualStatus: res.status,
      pass,
      response: res.body.substring(0, 200)
    };
    results.push(result);
    console.log(`${pass ? '✅' : '❌'} ${t.name}: ${t.size / 1024 / 1024}MB → ${res.status} (expected ${t.expected}) ${pass ? 'PASS' : 'FAIL'}`);
    if (!pass) console.log(`   Body: ${res.body.substring(0, 200)}`);
  }
  ...
  require('fs').writeFileSync('certification/v1.0.0-raw/50mb-edge-raw.json', JSON.stringify({
